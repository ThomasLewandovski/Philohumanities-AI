...
def _data_paths() -> Tuple[str, str]:
    s = get_settings()
    base = os.path.join(os.path.abspath(s.data_dir), "suggestions")
...
def _limit_sentences(text: str, max_sentences: int = 2) -> str:
    ...
    parts = re.split(r"([。！？!?]|\n)+", text.strip())
    # Reassemble keeping at most max_sentences endings
    out = []
    sentences = 0
...
async def generate_suggestions(cid: str, k: int = 4, max_sentences: int = 2, angles: List[str] | None = None, locale: str | None = None, diversify: bool = False) -> Dict[str, Any]:
    s = get_settings()
    ...
    # Build compact history: system + last few exchanges
